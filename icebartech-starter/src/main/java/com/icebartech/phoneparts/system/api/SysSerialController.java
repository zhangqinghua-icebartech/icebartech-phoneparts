...
public class SysSerialController extends BaseController {
    ...
    @Autowired
    public SysSerialController(SysSerialService service) {
    ...
    public RespDate<Boolean> excelInput(@RequestParam("file") MultipartFile file) throws Exception {
        ...
        List<SysSerialInsertParam> sysSerialInsertParams = ExcelUtils.getInstance().readExcel2Objects(inputStream, SysSerialInsertParam.class);
    ...
    @GetMapping("/excelOut")
    @RequireLogin(UserEnum.no_login)
    public void createExcelOut(HttpServletResponse response,
                               @RequestParam String randomStr) throws Exception {
        List<SysSerialDto> sysSerialDtos = service.findList(eq(SysSerialDto::getRandomStr,randomStr));
        ExcelUtils.getInstance().
                exportObjects2Excel(sysSerialDtos, SysSerialDto.class, true, null, true,response,"生成序列号列表");
    }
